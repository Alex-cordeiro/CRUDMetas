...
export class EmpresaService {
    ...
    retornaEmpresas(): Observable<Empresa[]> {
        return this.httpclient.get<Empresa[]>(`${environment.api}` + '/empresa')
    }

    retornaVendedoresPorIdEmpresa(idEmpresa: number): Observable<Vendedor[]> {
        return this.httpclient.get<Vendedor[]>(`${environment.api}` + '/vendedor/' + idEmpresa)
    }

    retornaDepartamentosPorIdEmpresa(idEmpresa: number): Observable<Departamento[]> {
        return this.httpclient.get<Departamento[]>(`${environment.api}` + '/departamento/' + idEmpresa)
    }

    retornaSetoresPorIdEmpresa(idEmpresa: number): Observable<Setor[]> {
        return this.httpclient.get<Setor[]>(`${environment.api}` + '/setor/' + idEmpresa)
    }
