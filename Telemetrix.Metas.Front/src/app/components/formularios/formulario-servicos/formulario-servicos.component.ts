...
export class FormularioServicosComponent implements OnInit {
  ...
  onSubmit(){
    if(this.formServico.valid){
      this.formulariopecasEnvio = {
        ...
        cpf: this.formServico.controls['cpf'].value,
        dataValidade: new Date(this.formServico.controls['dataValidade'].value)
      }
  ...
  public geraFormulario(servicosForm: ServicosForm){
    this.formServico = this.formBuilder.group({
      ...
      cpf: new FormControl(servicosForm.cpf),
      dataValidade: new FormControl(servicosForm.dataValidade)
    })
  }
